...
const SliderImmobiler = () => {
  ...
  useEffect(() => {
    if (subsubcategorySlug && subsubcategorySlug !== 'undefined') {
      // Estamos en nivel 3: sub-subcategoría específica
  ...
  const handleItemClick = (item) => {
    if (currentLevel === 1) {
      if (item.hasSublevel) {
        // Tiene subcategorías: mostrar nivel 2
  ...
  const getHeaderTitle = () => {
    if (currentLevel === 1) {
      return isMobile ? 'Immobilier' : 'Transactions Immobilières';
    } else {
      return selectedCategory?.name || 'Immobilier';
    }
  };

  // Obtener descripción según nivel
  const getHeaderDescription = () => {
    if (currentLevel === 1) {
      return 'Choisissez le type de transaction';
    } else {
      return `Sélectionnez un type de bien pour ${selectedCategory?.name}`;
    }
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            Sélectionnez une opération pour voir les types de biens disponibles
          </div>
        </div>
      </div>
