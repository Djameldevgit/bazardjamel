...
export const POST_CATEGORY_TYPES = {
    ...
    // Posts por categoría y subcategoría
...
export const getPostsByCategory = (category, page = 1, limit = 12) => async (dispatch) => {
    try {
        ...
        console.log(`📡 Cargando posts para categoría: ${category}, página: ${page}, limit: ${limit}`);
...
export const getPostsByCategoryHierarchy = (level1, level2 = null, level3 = null, page = 1, limit = 12) => async (dispatch) => {
    try {
        ...
        console.log(`📡 Llamando API: ${level1}${level2 ? '/' + level2 : ''}${level3 ? '/' + level3 : ''} - Página ${page}, Limit: ${limit}`);
        
        // ✅ Validar parámetros para evitar undefined
...
export const getSubCategoriesByCategory = (categorySlug) => async (dispatch) => {
    try {
        ...
        console.log(`📡 Buscando subcategorías para: ${categorySlug}`);
        
        const res = await getDataAPI(`categories/${categorySlug}/subcategories`);
        
        console.log('📥 Subcategorías encontradas:', res.data.subcategories?.length);
...
export const getPostsBySubcategory = (categorySlug, subcategorySlug, page = 1, limit = 12) => async (dispatch) => {
    try {
        ...
        console.log(`📡 Cargando posts para: ${categorySlug}/${subcategorySlug}, página: ${page}`);
        
        const res = await getDataAPI(`posts/category/${categorySlug}/subcategory/${subcategorySlug}?page=${page}&limit=${limit}`);
        
        console.log('📥 Respuesta subcategoría:', {
            success: res.data.success,
            postsCount: res.data.posts?.length
        });
...
// 📌 OBTENER POSTS POR JERARQUÍA DE CATEGORÍAS
...
// 📌 OBTENER SUB-SUBCATEGORÍAS (Nivel 3)
...
// 📌 ESTABLECER NIVEL ACTUAL DE CATEGORÍA
