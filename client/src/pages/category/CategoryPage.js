...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    if (currentLevel === 1) {
      // Nivel 1: mostrar subcategorías
      setChildrenToShow(children.filter(c => c.level === 2));
    } else if (currentLevel === 2) {
      // Nivel 2: mostrar artículos
      const articles = children.filter(c => c.level === 3);
  ...
  }, [children, currentLevel, articleSlug]);
  ...
  }, [slug, subSlug, articleSlug, postsLoading, dispatch]);
  ...
  const getPageTitle = () => {
    ...
    if (currentLevel === 2) return children?.find(c => c.slug === subSlug)?.name || subSlug;
  ...
  const buildBreadcrumbItems = () => {
    ...
    if (subSlug && currentLevel >= 2) items.push({ label: children?.find(c => c.slug === subSlug)?.name || subSlug, path: `/category/${slug}/${subSlug}` });
        ...
        <Container className="py-4">
          ...
          {shouldShowArticleSlider && (
            <ArticleSlider
              ...
              currentSubcategory={{ slug: subSlug, name: children?.find(c => c.slug === subSlug)?.name }}
